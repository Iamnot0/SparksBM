...
class ResponseFormatter:
    ...
    def format(result: Any, resultType: str = 'chat_response', context: Optional[Dict] = None) -> Any:
        ...
        # Check if result is already structured
        if isinstance(result, dict) and result.get('type') in ['table', 'object_detail']:
            return result
        
        # Check if result is formatted text
        ...
        if isinstance(result, str):
            # Check if string is actually a JSON representation of structured data
    ...
    def _formatError(error: Any, context: Optional[Dict] = None) -> str:
        ...
        if "LLM" in errorMsg or "API" in errorMsg:
    ...
    def _formatToolResult(result: Any, context: Optional[Dict] = None) -> str:
        ...
        if isinstance(result, dict):
            # Check for formatted text (from presenter) - return content
            if result.get('type') == 'text':
                return result.get('content', str(result))
            
            # Check for structured table data - return as-is for UI rendering
            ...
            # Check for common tool result structures
            if 'status' in result:
            ...
            # Excel/Word/PDF processing results
            if 'sheets' in result or 'data' in result:
                return ResponseFormatter._formatDocumentData(result)
    ...
    @staticmethod
    def _formatDocumentData(data: Dict) -> str:
        """Format Excel/Word/PDF document data"""
        output = []
        
        # Excel data
        if 'sheets' in data:
            output.append("📊 **Excel File Processed**\n")
            sheetNames = data.get('sheetNames', list(data.get('sheets', {}).keys()))
            output.append(f"**Sheets found:** {len(sheetNames)}\n")
            
            for sheetName in sheetNames:
                sheetData = data['sheets'].get(sheetName, {})
                rows = sheetData.get('rows', 0)
                cols = sheetData.get('columns', [])
                output.append(f"\n**📄 {sheetName}**")
                output.append(f"  • Rows: {rows}")
                output.append(f"  • Columns: {len(cols)}")
                if cols:
                    output.append(f"  • Column names: {', '.join(cols[:5])}{'...' if len(cols) > 5 else ''}")
            
            return "\n".join(output)
        
        # Word/PDF data
        if 'text' in data or 'paragraphs' in data:
            docType = "📄 **Word Document**" if 'paragraphs' in data else "📄 **PDF Document**"
            output.append(f"{docType} Processed\n")
            
            if 'paragraphCount' in data:
                output.append(f"**Paragraphs:** {data['paragraphCount']}")
            if 'pageCount' in data:
                output.append(f"**Pages:** {data['pageCount']}")
            if 'tableCount' in data:
                output.append(f"**Tables:** {data['tableCount']}")
            if 'totalCharCount' in data:
                output.append(f"**Characters:** {data['totalCharCount']:,}")
    ...
    def _formatString(text: str, context: Optional[Dict] = None) -> str:
        ...
        # Check if it's already formatted (contains markdown)
    ...
    def _formatSuccessData(data: Any, dataType: Optional[str] = None) -> str:
        ...
        if dataType == 'document':
            return ResponseFormatter._formatDocumentData(data) if isinstance(data, dict) else f"✅ **Document processed**\n\n{str(data)}"
