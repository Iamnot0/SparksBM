...
def setupAgent():
    """Setup and configure a document processing agent"""
    ...
    agent.registerTool('readExcel', excelTool.readExcel, 'Read Excel files')
    agent.registerTool('readWord', wordTool.readWord, 'Read Word documents')
    
    # Register LLM - use Gemini only
    try:
        # Use Gemini only
        ...
        # Continue without LLM - agent can still process files
    ...
    try:
        ...
        agent.registerTool('analyzeVeriniceObject', veriniceTool.analyzeObject, 'Analyze Verinice object using LLM')
...
def printMenu():
    ...
    print("=" * 70)
    print("\n[fa-file-excel] DOCUMENT PROCESSING:")
    print("   1. Process Excel file (.xlsx, .xls)")
    print("   2. Process Word document (.docx, .doc)")
...
def checkLLMStatus():
    ...
    if geminiKey:
        if Settings.GEMINI_MODEL == 'gemini-1.5-flash':
            print(f"  [+] Using gemini-1.5-flash (best for document processing)")
        else:
            print(f"  [*] Consider gemini-1.5-flash for faster, cost-effective document processing")
    else:
...
def showCapabilities(agent, executor):
    ...
    for toolName, toolInfo in agent.tools.items():
        ...
        elif toolName in ['readExcel', 'readWord']:
            doc_tools.append((toolName, desc))
    ...
    if doc_tools:
        print("\n  [fa-file-excel] Document Processing:")
        for toolName, desc in doc_tools:
...
def manageAgents(executor):
    ...
    if choice == '1':
        ...
        try:
            excelTool = ExcelTool()
            newAgent.registerTool('readExcel', excelTool.readExcel, 'Read Excel files')
            print("  [+] Registered readExcel")
        except:
            pass
...
def createVeriniceObject(executor, agent, objectType=None):
    ...
    # Get veriniceTool from agent (preferred) or create new instance
    veriniceTool = None
    ...
    # Get object type (if not provided)
    if not objectType:
    ...
    for idx, domain in enumerate(domains, 1):
        ...
        # Check if domain has subTypes for this object type
        hasSubTypes = False
    ...
    # Get valid subTypes for this object type in the domain
...
def analyzeVeriniceObject(executor, agent, objectType=None):
    """Analyze a Verinice object using LLM"""
    print("\n" + "=" * 70)
    print("Analyze Verinice Object (LLM)")
    print("=" * 70)
    print("[!]️  Analysis functionality - use chat mode for interactive analysis")
    print("   Or use: 'analyze <objectType> <objectId>' in chat")
...
def createVeriniceUnit(executor, agent):
    ...
    # Get domains (optional - can create unit without domains)
    domainIds = []
    domainsResult = veriniceTool.listDomains()
...
2. Agent Executor → Document Agent
   └─> Executor selects agent and calls agent.process()

3. Document Agent analyzes input:
   ├─> If file path → _processFile()
   │   └─> Detects file type → Calls appropriate tool
   │       ├─> Excel → readExcel()
   │       └─> Word → readWord()
   │
   ├─> If text message → _processChatMessage()
   │   └─> Uses LLM tool (generate/analyze)
   │
   └─> If dict → _processData()
       └─> Processes already structured data
...
def processFile(executor, filePath):
    """Process a file"""
    print(f"\nProcessing file: {filePath}")
    try:
        result = executor.execute(
            task=f"Process document: {filePath}",
            inputData=filePath
        )
        ...
        if result.get('success'):
            ...
            if isinstance(resultData, dict):
                if resultData.get('status') == 'success':
                    print(f"  Status: {resultData.get('status')}")
                    data = resultData.get('result', {})
                    if isinstance(data, dict):
                        if 'sheets' in data:
                            print(f"\n  Sheets found: {', '.join(data.get('sheetNames', []))}")
                            for sheetName, sheetData in data.get('sheets', {}).items():
                                print(f"\n  [#] Sheet: {sheetName}")
                                print(f"    Rows: {sheetData.get('rows', 0)}, Columns: {len(sheetData.get('columns', []))}")
                                print(f"    Columns: {', '.join(sheetData.get('columns', []))}")
                                ...
                                if rows:
                                    ...
                                    if rows:
                                        ...
                                        headerRow = " | ".join(str(h)[:15].ljust(15) for h in headers)
                                        print(f"    {headerRow}")
                                        print("    " + "-" * 66)
                                        # Show data rows (limit to first 10)
                                        for i, row in enumerate(rows[:10], 1):
                                            rowStr = " | ".join(str(v)[:15].ljust(15) for v in row.values())
                                            print(f"    {rowStr}")
                                        if len(rows) > 10:
                ...
                elif resultData.get('status') == 'error':
                    print(f"[-] Error: {resultData.get('error', 'Unknown error')}")
        else:
            print(f"\n[-] Execution failed")
            ...
            if isinstance(resultData, dict):
                nestedError = resultData.get('error')
                if nestedError:
...
def chatWithLLM(executor, agent):
    ...
    print("You can ask questions or give commands like 'process file.xlsx', 'show capabilities', etc.")
    print("=" * 70)
    print()
    ...
    while True:
        ...
        try:
            ...
            if result.get('success'):
                resultData = result.get('result', {})
                if isinstance(resultData, dict):
                    # Check if it's an action to execute
                    if resultData.get('type') == 'action':
                        ...
                        if actionType == 'process_file':
                            filePath = action.get('filePath', '')
                            if filePath:
                                print(f">>> Processing {filePath}...\n")
                                processFile(executor, filePath)
                                print()
                                print(f">>> [+] File processed! You can now ask me to 'analyze' it, 'show capabilities', or process another file.\n")
                            else:
                                print(f">>> No file specified. Please provide a file path like 'process uploads/sample_data.xlsx' or use menu option 1.\n")
                        elif actionType == 'show_capabilities':
                            print()
                            ...
                            if not hasattr(agent, '_veriniceTool') or not agent._veriniceTool or not agent._veriniceTool._checkClient():
                                ...
                                try:
                                    if actionType == 'verinice_create':
                                    ...
                                    elif actionType == 'verinice_analyze':
                                        analyzeVeriniceObject(executor, agent, objectType=detectedObjectType)
                                        ...
                                        print(f">>> Available: create, list, view, update, delete, generate report, list reports, analyze, list domains, list units\n")
                                except Exception as e:
                    ...
                    # Handle raw data responses (like file processing results)
                    if isinstance(resultData, dict):
                        if 'sheets' in resultData:
                            print(f">>> [+] File processed successfully! I've extracted data from {len(resultData.get('sheetNames', []))} sheet(s).\n")
                            print(f">>> You can ask me to 'analyze' it, 'show capabilities', or process another file.\n")
                        elif 'text' in resultData:
                            textLen = len(resultData.get('text', ''))
                            print(f">>> [+] File processed successfully! I've extracted {textLen} characters of text.\n")
                            print(f">>> You can ask me to 'analyze' it, 'show capabilities', or process another file.\n")
                        else:
                            print(f">>> {resultData}\n")
                    else:
                ...
                if not errorMsg or errorMsg == 'Unknown error':
                    ...
                    print(f">>>   - 'process file.xlsx' to process a file\n")
                    print(f">>>   - 'yes' if I asked you something\n")
                elif "LLM" in errorMsg:
                    print(f">>> LLM service unavailable. I can still process files and answer quick questions!\n")
                    print(f">>> Try: 'show capabilities', 'process file.xlsx', or 'check LLM status'\n")
                else:
                    print(f">>> {errorMsg}\n")
        except Exception as e:
            import traceback
            ...
            print(f">>> Sorry boss, something went wrong. Try: 'show capabilities' or 'process file.xlsx'\n")
...
def testWorkflow(executor):
    ...
    sampleFile = "uploads/sample_data.xlsx"
    if not os.path.exists(sampleFile):
        print(f"[!] Sample file not found: {sampleFile}")
        ...
        try:
            ...
            df.to_excel(sampleFile, index=False)
            print(f"[+] Created {sampleFile}")
        except Exception as e:
            print(f"[-] Failed to create sample: {e}")
            return
    
    steps = [
        {'task': 'Read Excel file', 'input': sampleFile},
        {'task': 'Extract data', 'input': None}  # Will use previous result
    ]
    
    print(f"\nExecuting workflow with {len(steps)} steps...")
    result = executor.executeWorkflow(steps)
    
    print(f"\nWorkflow Result:")
    print(f"  Success: {result.get('success')}")
    print(f"  Completed: {result.get('completedSteps')}/{result.get('totalSteps')}")
    print(f"\nSteps:")
    for step in result.get('steps', []):
        status = "[+]" if step['result'].get('success') else "[-]"
        print(f"  {status} Step {step['step']}: {step['task']}")
    print("=" * 70)
...
def main():
    ...
    print("=" * 70)
    print("Agentic Framework - Document Processing Agent")
    print("=" * 70)
    ...
    # Check for file path argument (non-interactive mode)
    if len(sys.argv) > 1:
        filePath = sys.argv[1]
        processFile(executor, filePath)
        return
    ...
    while True:
        ...
        if choice == '0':
            ...
            filePath = input("Enter Word document path: ").strip()
            if filePath:
                processFile(executor, filePath)
            else:
                print("[!] No file path provided")
            ...
            analyzeVeriniceObject(executor, agent)
        elif choice == '10':
            listVeriniceDomains(executor, agent)
        elif choice == '11':
