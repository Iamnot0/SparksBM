...
def checkGreeting(message: str, processedCount: int = 0) -> Optional[str]:
    ...
    Args:
        ...
        processedCount: Number of documents processed (for personalized greeting)
    ...
    # Check for greetings using regex (faster than list iteration)
    if re.match(GREETING_PATTERN, messageLower, re.IGNORECASE):
        if processedCount > 0:
            return f"Hi! I've processed {processedCount} document(s) and I'm ready to help. What would you like me to do?"
        else:
            return "Hi! I'm ready to help with documents and ISMS operations."
    
    # Check for gratitude using regex
