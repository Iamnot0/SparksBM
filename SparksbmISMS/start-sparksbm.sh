...
docker compose up -d 2>/dev/null || docker-compose up -d 2>/dev/null || { echo -e "${RED}✗${NC} Failed to start Docker services"; exit 1; }
...
if curl -s $BACKEND_URL/actuator/health > /dev/null 2>&1 || curl -s $BACKEND_URL/swagger-ui.html > /dev/null 2>&1; then
    ...
    ./gradlew veo-rest:bootRun -PspringProfiles=local > /tmp/sparksbm-backend.log 2>&1 &
    BACKEND_PID=$!
